...
public class BuggySearchAndSort {
	...
	public static boolean contains(int[] array, int val) {
		for (int i = 0; i < array.length; i++) {
			if (array[i] == val)
				return true;		//	Bug #1: after looking at A[0], the subroutine already returns
//			else						either true or false. i++ never happens, so the subroutine
//				return false;		//	really says if the first element of array is equal to val.
		}
		return false;
	}
	...
	public static void bubbleSort(int[] array) {
		for (int i = 0; i < array.length; i++) {
			for (int j = 0; j < array.length-1; j++) { // Bug #2 - need to replace i++ with j++
	...
	public static void selectionSort(int[] array) {
		for (int top = array.length - 1; top > 0; top--) {
			int positionOfMax = 0;
			for (int i = 1; i <= top; i++) {
				if (array[i] > array[positionOfMax]) // Bug #3 - replace array[1] with array[i]
					positionOfMax = i;				 // otherwise there is no progress through the array
			}
	...
	public static void insertionSort(int[] array) {
		for (int top = 1; top < array.length; top++) {
			...
			int pos = top - 1;
			while (pos >= 0 && array[pos] > temp) {    // Bug #4 - the loop has to start with the beginning of the array,
				// so pos >= needs to be where pos > 0 is
				// move items that are bigger than temp up one position
				array[pos + 1] = array[pos];
				pos--;
			}
			array[pos + 1] = temp;  // place temp into last vacated position // and here comes bug #5 - [pos+1] should be
		}																   // where [pos] is. It causes ArrayIndexOutOfBoundsException
	}
